...
export const getArtisanById = async (id) => {
  try {
    ...
    console.error(
      `SERVICE LOG: Erro ${error.response?.status} ao buscar artesão ${id}:`,
...
export const saveArtisan = async (userData, id) => {
  try {
    const response = await api.patch(`/artisans/${id}`, userData);
    return response.data;
  } catch (error) {
