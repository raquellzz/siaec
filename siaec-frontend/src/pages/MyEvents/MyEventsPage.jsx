...
function MyEventsPage() {
  ...
  if (error)
    return (
      <div className="my-events-container" style={{ color: 'red' }}>
        {error}
      </div>
    );
  ...
  return (
    <div className="my-events-container">
      ...
      <div className="event-tabs">
        {tabs.map((tab) => (
          <button key={tab} className={`tab-button ${filter === tab ? 'active' : ''}`} onClick={() => setFilter(tab)}>
            {tab === 'Próximo'
              ? 'Próximos'
              : tab === 'Concluído'
              ? 'Concluídos'
              ...
              <div key={event.eventId} className="event-card">
                <div className="card-header">
                  ...
                  <div className="card-actions">
                    ...
                    <button className="icon-btn delete" title="Excluir" onClick={() => handleDelete(event.eventId)}>
                      <svg
                        width="18"
                        height="18"
                        viewBox="0 0 24 24"
                ...
                <div className="card-footer">
                  ...
                  <span className={`status-tag ${getStatusStyle(currentStatus)}`}>{currentStatus}</span>
                </div>
              </div>
            );
          })
